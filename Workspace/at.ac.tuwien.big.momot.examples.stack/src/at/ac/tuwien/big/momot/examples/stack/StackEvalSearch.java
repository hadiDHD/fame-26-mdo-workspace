...
public class StackEvalSearch {
   ...
   public static void main(final String[] args) {
      ...
      manager.removeUnits(StackModule.CreateStack.NAME, StackModule.ConnectStacks.NAME); // defined in same file, but
                                                                                         // not applicable
      ...
      manager.addNonSolutionParameters(StackModule.ShiftLeft.Parameter.FROM_LOAD,
            StackModule.ShiftLeft.Parameter.TO_LOAD, StackModule.ShiftRight.Parameter.TO_LOAD,
      ...
      search.getFitnessFunction().addObjective(new AbstractEGraphFitnessDimension("Standard Deviation") {
         ...
         protected double internalEvaluate(final TransformationSolution solution) {
            final StackModel root = MomotUtil.getRoot(solution.execute(), StackModel.class);

            final List<Integer> loads = new ArrayList<>();
            for(final Stack stack : root.getStacks()) {
